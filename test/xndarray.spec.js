...
describe('xndarray constructor', () => {
  it('should work', () => {
    let arr = xndarray([1,2,3,4], {shape: [2,2], names: ['y','x'], domains: {x: [0,1], y: [5,6]}})
    ...
    assert.strictEqual(arr.domains.length, 2)
    assert.strictEqual(arr.xdomain('y').get(0), 5)
    assert.strictEqual(arr.domains[0].get(0), 5)
  })
  it('should work with alternative domains format', () => {
    let arr = xndarray([1,2,3,4], {shape: [2,2], domains: [[5,6],[0,1]]})
    assert.deepEqual(arr.data, [1,2,3,4])
    assert.deepEqual(arr.shape, [2,2])
    assert.strictEqual(arr.domains.length, 2)
    assert.strictEqual(arr.domains[0].get(0), 5)
  })
  it('should work without names or domains', () => {
    let arr = xndarray([1,2,3,4], {shape: [2,2]})
    
    // auto-generated dimension names and domain values
    assert.deepEqual(arr.names, ['dim_0', 'dim_1'])
    assert.deepEqual(unpack(arr.domains[0]), [0,1])
    assert.deepEqual(unpack(arr.domains[1]), [0,1])
  })
  it('should work with ndarray bidirectionally', () => {
    let nd = ndarray([1,2,3,4], [2,2])
    let arr = xndarray(nd, {domains: [[5,6],[0,1]]})
    assert.deepEqual(arr.data, [1,2,3,4])
    assert.deepEqual(arr.shape, [2,2])
    assert.strictEqual(arr.domains.length, 2)
    assert.strictEqual(arr.domains[0].get(0), 5)
  ...
  describe('Slicing methods', () => {
    describe('#lo', () => {
      it('should return an xndarray object again', () => {
        let arr = xndarray([1,2,3,4], {shape: [2,2], names: ['y','x'], domains: {y: [0,1], x: [2,3]}})
        let sliced = arr.lo(1, 1)
        assert(sliced.names)
        assert(sliced.xget)
        
        assert.strictEqual(sliced.xdomain('y').size, 1)
        assert.strictEqual(sliced.xdomain('x').size, 1)
        assert.strictEqual(sliced.xdomain('y').get(0), 1)
        assert.strictEqual(sliced.xdomain('x').get(0), 3)
      })
    })
    describe('#xlo', () => {
      it('should work correctly', () => {
    ...
    describe('#xhi', () => {
      it('should work correctly', () => {
        ...
        assert.strictEqual(sliced.xdomain('y').size, 1)
        assert.strictEqual(sliced.xdomain('x').size, 1)
        assert.strictEqual(sliced.xdomain('y').get(0), 0)
        assert.strictEqual(sliced.xdomain('x').get(0), 2)
      })
    })
    describe('#xstep', () => {
      it('should work correctly', () => {
        let arr = xndarray([1,2,3,4,5,6], {names: ['x'], domains: {x: [0,1,2,3,4,5]}})
        ...
        assert.deepEqual(unpack(evens.xdomain('x')), [0,2,4])
      })
    })
    describe('#transpose', () => {
      it('should return an xndarray object again', () => {
        let arr = xndarray([1,2,3,4,5,6], {shape: [2,3], names: ['y','x'], domains: {y: [0,1], x: [2,3,4]}})
        let transposed = arr.transpose(1, 0)
        assert.deepEqual(transposed.names, ['x','y'])
        assert.deepEqual(transposed.shape, [3,2])
        
        assert.deepEqual(unpack(transposed.xdomain('y')), [0,1])
        assert.deepEqual(unpack(transposed.xdomain('x')), [2,3,4])
      })
    })
    describe('#xtranspose', () => {
      it('should work correctly', () => {
    ...
    describe('#pick', () => {
      it('should return an xndarray object again', () => {
        ...
        assert(sliced.xdomain('y'))
        assert(sliced.xdomain('x'))
        assert(!sliced.xdomain('t'))
      })
    })
    describe('#xpick', () => {
      it('should work correctly', () => {
